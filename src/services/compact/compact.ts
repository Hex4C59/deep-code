...
export async function compactConversation(
  ...
  try {
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 5 })
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 15 })
    ...
    for (;;) {
      ...
      context.onCompactProgress?.({ type: 'compact_progress', progress: 72 })
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 76 })
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 84 })
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 90 })
    ...
    context.onCompactProgress?.({ type: 'compact_progress', progress: 98 })
    ...
    context.setStreamMode?.('requesting')
    context.setResponseLength?.(() => 0)
    context.onCompactProgress?.({ type: 'compact_end', completed: succeeded })
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      while (!next.done) {
        ...
        if (
          ...
          streamedCharacters += charactersStreamed
          context.setResponseLength?.(length => length + charactersStreamed)
          context.onCompactProgress?.({
            type: 'compact_progress',
            progress: Math.min(68, 18 + Math.floor(streamedCharacters / 60)),
          })
        }
...
function shouldExcludeFromPostCompactRestore(
  ...
  // and to also match child directory memory files (.close/rules/*.md, etc.)
  try {
