...
function isStdioMcpServer(
  config: ScopedMcpServerConfig,
): config is McpStdioServerConfig & ScopedMcpServerConfig {
...
export const connectToServer = memoize(
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
      ...
      } else if (isStdioMcpServer(serverRef) && isClaudeInChromeMCPServer(name)) {
      ...
      } else if (isStdioMcpServer(serverRef)) {
        const finalCommand =
        ...
        const unsupportedServerRef = serverRef as ScopedMcpServerConfig
        throw new Error(`Unsupported server type: ${unsupportedServerRef.type}`)
      }
...
/**
 * Call an MCP tool directly as an RPC
 * @param toolName The name of the tool to call
 * @param args The arguments to pass to the tool
 * @param client The MCP client to use for the RPC call
...
export async function processMCPResult(
  result: unknown,
  tool: string, // Tool name for validation (e.g., "search")
  name: string, // Server name for validation and transformation (e.g., "slack")
