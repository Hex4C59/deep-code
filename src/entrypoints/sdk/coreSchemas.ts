  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'legacy-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.close/agent-memory/<agentType>/, 'project' - .close/agent-memory/<agentType>/, 'local' - .close/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.close/settings.json). " +
        "'project' - Project settings (.close/settings.json). " +
        "'local' - Local settings (.close/settings.local.json).",
    ),
)
