...
/**
 * Kind override from env. Set by the spawner (`close --bg`, daemon
...
/**
 * True when this REPL is running inside a `close --bg` tmux session.
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.close/sessions/ is shared with Windows-native Claude (symlink
