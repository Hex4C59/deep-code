...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const claudeDir = join(home, '.close')
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: close completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const closeBin = process.argv[1] || 'close'
  const result = await execFileNoThrow(closeBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: close completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('close completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.close/.
 * Called after `close update` so completions stay in sync with the new binary.
 */
