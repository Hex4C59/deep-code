// Scheduled prompts, stored in <project>/.close/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.close', 'scheduled_tasks.json')
...
/**
 * Read and parse .close/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .close/scheduled_tasks.json with the given tasks. Creates .close/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.close'), { recursive: true })
 ...
 * written to .close/scheduled_tasks.json and dies with the process. The
