// Scheduler lease lock for .close/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.close', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .close/ doesn't exist yet — create it and retry once. In steady
