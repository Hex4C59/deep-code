...
function modelBelongsToFamily(model: string, family: string): boolean {
  ...
  // Resolve aliases like "best" to check family membership
 ...
 * e.g. "opus-4-5" matches "opus-4-5-20251101" but not "opus-4-50".
 */
 ...
 * Supports prefixes like "opus-4-5". Resolves input aliases before matching.
 */
function modelMatchesVersionPrefix(model: string, entry: string): boolean {
  ...
  // Try the entry as-is (e.g. "opus-4-5")
...
function familyHasSpecificEntries(
  ...
  for (const entry of allowlist) {
    ...
    // e.g., "opus-4-5" or "opus-4-5-20251101" for the "opus" family
