 ...
 * user-controlled files (~/.close/settings.json and ~/.close.json),
 ...
 *
 * We read from global config (~/.close.json) and user settings
 * (~/.close/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.close/settings.json),
