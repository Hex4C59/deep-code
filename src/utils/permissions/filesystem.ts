...
/**
 * If filePath is inside a .close/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .close/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.close', 'skills')),
      prefix: '/.close/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.close', 'skills')),
      prefix: '~/.close/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.close/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.close/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.close${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.close${sep}settings.local.json`)
  ) {
    // Include .close/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .close/commands or .close/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.close', 'commands')
  const agentsDir = join(getOriginalCwd(), '.close', 'agents')
  const skillsDir = join(getOriginalCwd(), '.close', 'skills')
 ...
 * Path format: ~/.close/projects/{sanitized-cwd}/...
 */
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getClaudeTempDir(), 'bundled-skills', '0.0.1', nonce)
  },
)
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .close/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .close
      // segment when it's followed by 'worktrees'. Any nested .close directories
      ...
      if (dir === '.close') {
        ...
        if (
          ...
          break // Skip this .close, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .close , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .close , .bashrc..., settings.json.
 ...
 * - Claude config files (.close/settings.json, .close/commands/, .close/agents/)
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .close/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .close is a dangerous directory
  ...
  // 1.6. Check for .close/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .close/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .close/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.close) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .close/ (project or global).
    // Accepts both the broad patterns ('/.close/**', '~/.close/**') and
    // narrowed ones like '/.close/skills/my-skill/**' so users can grant
    ...
    // '/.close/../**' from leaking this bypass outside .close/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .close/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.close/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.close/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.close is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.close/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .close/launch.json — desktop preview config (dev server command + port).
  ...
  // .close/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .close/ only (not ~/.close/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.close', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.close/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.close/teams/) for swarm coordination
