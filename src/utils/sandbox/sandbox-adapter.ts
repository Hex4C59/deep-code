...
export function convertToSandboxRuntimeConfig(
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.close', 'settings.json'))
    denyWrite.push(resolve(cwd, '.close', 'settings.local.json'))
  }

  // Block writes to .close/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .close/commands and
  // .close/agents but not .close/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Claude capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.close', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.close', 'skills'))
  }
