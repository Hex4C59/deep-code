...
async function updateLatest(
  ...
  if (!forceReinstall) {
    ...
    if (maxVersion && gt(version, maxVersion)) {
      ...
      if (gte('0.0.1', maxVersion)) {
        logForDebugging(
          `Native installer: current version ${'0.0.1'} is already at or above maxVersion ${maxVersion}, skipping update`,
        )
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Check for local installation at ~/.close/local
  const localInstallDir = join(homedir(), '.close', 'local')
