 ...
 * env vars that the remote's ~/.close settings.env MUST NOT clobber (see
 ...
 * user's ~/.close/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.close/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.close.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.close/settings.json env (gh#217). policy/flag
