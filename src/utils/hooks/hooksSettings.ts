...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.close/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.close/settings.json)'
    case 'projectSettings':
      return 'Project settings (.close/settings.json)'
    case 'localSettings':
      return 'Local settings (.close/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.close/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.close/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
