 ...
 * 2. User memory (~/.close/AGENTS.md) - Private global instructions for all projects
 * 3. Project memory (AGENTS.md, .close/AGENTS.md, and .close/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - AGENTS.md, .close/AGENTS.md, and all .md files in .close/rules/ are checked in each directory for Project memory
 *
...
/**
 * Checks whether an AGENTS.md file path is excluded by the claudeMdExcludes setting.
...
/**
 * Processes all .md files in the .close/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .close/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.close/rules/*.md files
    ...
    // .close/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like AGENTS.md and .close/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .close/AGENTS.md (Project)
        const dotClaudePath = join(dir, '.close', 'AGENTS.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .close/rules/*.md files (Project)
        const rulesDir = join(dir, '.close', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .close/AGENTS.md from the additional directory
        const dotClaudePath = join(dir, '.close', 'AGENTS.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .close/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.close', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .close/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .close/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (AGENTS.md and .close/AGENTS.md)
  ...
  // Process project unconditional .close/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .close/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .close
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .close
...
/**
 * Check if a file path is a memory file (AGENTS.md, AGENTS.local.md, or .close/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .close/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.close${sep}rules${sep}`)
  ) {
    return true
  }
