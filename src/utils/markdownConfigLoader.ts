 ...
 * stopping there makes the parent project's `.close/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.close/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .close directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.close/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .close/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.close', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.close', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .close/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .close/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .close/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.close is
 ...
 * @param dir Directory (eg. "~/.close/commands")
 * @returns Array of parsed markdown files with metadata
 */
