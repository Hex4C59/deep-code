 ...
 * to the user's ~/.close/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  if (!isEnvDefinedFalsy(process.env.CLOSE_CODE_USE_OPENAI_COMPAT)) return false
  if (isEnvTruthy(process.env.CLAUDE_CODE_USE_OPENAI_COMPAT)) return false
...
export function getAnthropicApiKeyWithSource(
  ...
  // Always check for direct environment variable when the user ran close --print.
 ...
 * from ~/.close/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.close.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.close.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.close.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
