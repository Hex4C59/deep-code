 ...
 * Matches anything starting with a lowercase XML-like tag (hook
...
export function getFirstMeaningfulUserMessageTextContent<T extends Message>(
  ...
  for (const msg of transcript) {
    ...
    // Collect all text values. For array content, iterate all text blocks so
    // we don't miss the real prompt hidden behind metadata tags.
    const texts: string[] = []
    ...
    for (const textContent of texts) {
      ...
      // autonomous tick prompts, task notifications, pure metadata tags)
...
async function readLiteMetadata(
  ...
  // and catch array-format content blocks with metadata.
  const firstPrompt =
...
function extractFirstPromptFromChunk(chunk: string): string {
  ...
  while (start < chunk.length) {
    ...
    try {
      ...
      // iterate all text blocks so we don't miss the real prompt hidden behind
      // metadata tags.
      const texts: string[] = []
      if (typeof content === 'string') {
        texts.push(content)
