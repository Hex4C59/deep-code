...
export function PackageManagerAutoUpdater(t0) {
    ...
    t1 = async () => {
      ...
      if (maxVersion && latest && gt(latest, maxVersion)) {
        ...
        if (gte('0.0.1', maxVersion)) {
          logForDebugging(`PackageManagerAutoUpdater: current version ${'0.0.1'} is already at or above maxVersion ${maxVersion}, skipping update`);
          setUpdateAvailable(false);
          return;
        }
        latest = maxVersion;
      }
      const hasUpdate = latest && !gte('0.0.1', latest) && !shouldSkipVersion(latest);
      setUpdateAvailable(!!hasUpdate);
      if (hasUpdate) {
        logForDebugging(`PackageManagerAutoUpdater: Update available ${'0.0.1'} -> ${latest}`);
      }
    };
    $[0] = t1;
  } else {
  ...
  let t4;
  if ($[3] !== verbose) {
    t4 = verbose && <Text dimColor={true} wrap="truncate">currentVersion: {'0.0.1'}</Text>;
    $[3] = verbose;
    $[4] = t4;
  } else {
    t4 = $[4];
