...
export type Props = {
  ...
  headerText?: string
  /** Optional provider-layer shortcut shown for OpenAI-compatible model picking. */
  onChangeProvider?: () => void
  /**
   * When true, skip writing effortLevel to userSettings on selection.
   * Used by the assistant installer wizard where the model choice is
   * project-scoped (written to the assistant's .close/settings.json via
   * install.ts) and should not leak to the user's global ~/.close/settings.
   */
...
export function ModelPicker({
  ...
  const handleCycleEffort = useCallback(
    (direction: 'left' | 'right') => {
      ...
      const model = resolveOptionModel(focusedValue) ?? getDefaultMainLoopModel()
      setEffort(prev =>
        cycleEffortLevel(
  ...
  useInput(
    (input, key) => {
      if (onChangeProvider && key.ctrl && input.toLowerCase() === 'a') {
        onChangeProvider()
      }
    },
    { isActive: !!onChangeProvider },
  )
      ...
      <Box flexDirection="column">
        <Box marginBottom={1} flexDirection="column">
          ...
          <Text dimColor>
            {headerText ??
              (isOpenAICompat
                ? 'Switch the OpenAI-compatible model for this session and future sessions. For another model name, specify with --model.'
...
function EffortLevelIndicator({
  ...
  return <Text color={color}>{effortLevelToSymbol(level)}</Text>
}
...
function cycleEffortLevel(
  ...
  const levels = getSelectableEffortLevels(model).filter(
    level => includeMax || (level !== 'max' && level !== 'xhigh'),
  )
