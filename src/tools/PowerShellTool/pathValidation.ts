...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .close is a dangerous directory
  // and internal editable paths live under ~/.close/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.close; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.close/settings.json (Claude's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.close`, validate subsequent statements with
  // cwd='./.close'. This would be more permissive but requires careful
  // handling of:
