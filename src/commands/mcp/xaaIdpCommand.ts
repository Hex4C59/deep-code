/**
 * `close mcp xaa` — manage the XAA (SEP-990) IdP connection.
 *
...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          "Error: no XAA IdP connection. Run 'close mcp xaa setup' first.",
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : "no — run 'close mcp xaa login'"}\n`,
      )
      cliOk()
    })
