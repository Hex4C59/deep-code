...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires legacy subscription authentication and is disabled in Close Code.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires legacy full-scope authentication and is disabled in Close Code.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine Remote Control eligibility because legacy auth is disabled in Close Code.'
    }
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt('0.0.1', config.minVersion)) {
      return `Your version of Claude Code (${'0.0.1'}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`claude update\` to update.`
    }
  }
  return null
}
