...
function patchFile(filePath) {
  ...
  let changed = false
  const bunBundleImport = path
    .relative(path.dirname(filePath), path.join(ROOT, 'stubs', 'bun-bundle.js'))
    .replaceAll(path.sep, '/')
  const normalizedBunBundleImport = bunBundleImport.startsWith('.')
    ? bunBundleImport
    : `./${bunBundleImport}`

  // 1. Replace `import { feature } from 'bun:bundle'` / `"bun:bundle"`
  if (
    src.includes("from 'bun:bundle'") ||
    src.includes('from "bun:bundle"') ||
    /from\s*['"][./]*stubs\/bun-bundle\.js['"]/.test(src)
  ) {
    src = src.replace(
      /import\s*\{\s*feature\s*\}\s*from\s*['"](?:bun:bundle|[./]*stubs\/bun-bundle\.js)['"]/g,
      `import { feature } from '${normalizedBunBundleImport}'`,
    )
    changed = true
  }
